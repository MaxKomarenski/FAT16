...
std::vector<int> convert_to_int(const std::vector<uint8_t> &v){
    std::vector<int> test;
...
std::vector<std::vector<uint8_t >> get_all_files(std::vector<uint8_t> &bytes, uint16_t start, int size){
...
int main(){
    ...
    for(int i=0; i<all_files.size(); ++i){
        ...
        std::vector<uint8_t> sizeOfFile = get_info(bytes, fileOption["size of file"]);
        std::vector<uint8_t> creationFileTime = get_info(bytes, fileOption["creation time"]);
        std::vector<uint8_t> modifiedFileTime = get_info(bytes, fileOption["modified time"]);
        std::vector<uint8_t> attributes = get_info(bytes, fileOption["attributes"]);
        std::vector<uint8_t> numOfFirstCluster = get_info(bytes, fileOption["number of first cluster"]);
